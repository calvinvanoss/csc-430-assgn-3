...
(define (subst what for in)
  (match in
    ...
    [(idC s) (cond
               [(eq? s for) (numC what)]
               [else in])]
    [(appC fun args) (appC fun (map (lambda (arg) (subst what for (cast arg ExprC))) args))]
...
(check-equal? (subst 3 'f (appC 'g (list (idC 'f)))) (appC 'g (list (numC 3))))
(check-equal? (subst 3 'f (binopC '- (numC 1) (numC 2))) (binopC '- (numC 1) (numC 2)))
(check-equal? (subst 3 'f (binopC '* (numC 1) (numC 2))) (binopC '* (numC 1) (numC 2)))
(check-equal? (subst 3 'f (binopC '/ (numC 1) (numC 2))) (binopC '/ (numC 1) (numC 2)))
(check-equal? (subst 3 'f (ifleq0? (idC 'f) (numC 1) (numC 0))) (ifleq0? (numC 3) (numC 1) (numC 0)))
  ...
  (match exprc
    ...
    [(appC fun args) (let ([fd (get-fundef fun fds)])
                       (if (eq? (length args) (length (fundefC-args fd)))
                           (interp (foldl
                                    (lambda (arg param result)
                                      (subst (interp (cast arg ExprC) fds) (cast param Symbol) (cast result ExprC)))
                                    (fundefC-body fd) args (fundefC-args fd)) fds)
